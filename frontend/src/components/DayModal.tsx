...
export function DayModal({
  ...
  // Mobile: Centered floating card design
  if (isMobile) {
    return (
      <>
        <div
        ...
        >
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm max-h-[75vh] overflow-hidden">
            {/* Modal Header */}
            <div className="flex items-center justify-between p-4 border-b border-gray-100 bg-white">
              <div className="flex-1 min-w-0">
                <h2 className="text-lg font-medium text-gray-900 truncate">
                  {selectedMemory ? "Memory Details" : formatDate(date)}
                </h2>
                    ...
                    <div className="space-y-3">
                      {memories.map((memory) => (
                        <button
                          ...
                          className={`w-full text-left p-3 rounded-xl border transition-all duration-200 hover:shadow-sm ${getEmotionColor(
                            memory.emotions
                          )}`}
                        >
                          <div className="flex items-start justify-between mb-2">
                            <div className="flex items-center space-x-2">
                              {memory.importance > 0.7 && (
                                <div className="text-xs px-2 py-0.5 bg-purple-100 text-purple-700 rounded-full font-medium">
                                  Important
                                </div>
                              )}
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          {selectedMemory.photos.map((photo, index) => (
                            <div key={index} className="relative group">
                              ...
                              />
                              <div className="absolute inset-0 bg-black bg-opacity-0 group-active:bg-opacity-20 transition-opacity rounded-lg flex items-center justify-center pointer-events-none">
                                <svg
                                  className="h-5 w-5 text-white opacity-0 group-active:opacity-100 transition-opacity"
                                  fill="none"
                                  viewBox="0 0 24 24"
                                  stroke="currentColor"
                                >
                      ...
                      <div className="bg-gray-50 rounded-lg p-3">
                        <div className="flex items-center justify-between text-xs">
                          <span className="text-gray-600">
                            {selectedMemory.emotions.joy >
                            selectedMemory.emotions.sadness
                              ? "😊 Positive"
                              : selectedMemory.emotions.sadness > 0.3
                              ? "😔 Reflective"
                              : "😐 Neutral"}
                          </span>
                          <span className="text-gray-500">
                            {selectedMemory.emotions.intensity > 0.7
                              ? "Strong"
                              : "Moderate"}
                          </span>
                        </div>
  ...
  return (
    <>
      ...
      >
        <div
          className={`bg-white rounded-2xl shadow-2xl max-h-[80vh] transition-all duration-300 ${
        ...
        >
          ...
          <div className="flex">
            ...
            <div
              className={`${
                selectedMemory ? "w-1/2 border-r border-gray-100" : "w-full"
      ...
      {/* Image Modal Overlay for Desktop */}
      {showImageModal && (
        <div
          className="fixed inset-0 bg-black bg-opacity-90 flex items-center justify-center z-[60] p-4"
          onClick={closeImageModal}
